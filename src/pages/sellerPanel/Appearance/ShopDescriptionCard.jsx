...
import { getWebsiteById, updateWebsitePartial } from '../../../API/website';
...
const ShopDescriptionCard = () => {
  ...
  useEffect(() => {
    const fetchWebsiteData = async () => {
      if (!websiteId) return console.warn("آیدی فروشگاه در URL پیدا نشد");

      try {
        const website = await getWebsiteById(websiteId);
        if (website?.guide_page) {
  ...
  const handleSave = async () => {
    if (description.trim() === '') {
      ...
      showErrorModal("لطفاً توضیحاتی در مورد فروشگاه خود وارد کنید");
      return;
    }
    ...
    try {
      ...
      await updateWebsitePartial(websiteId, {
        ...
        <div className="mt-6 flex flex-col gap-4 rounded-xl p-5 relative">
          ...
          <div className="relative">
            <textarea
              ...
              className={`w-full h-36 p-4 border rounded-lg resize-none transition-all duration-200 ${
                !isEditing ? 'bg-gray-50 border-gray-300 text-gray-600 cursor-not-allowed' : 'bg-white border-gray-300 text-gray-800'
              }`}
              disabled={!isEditing}
            />
