...
const Header = () => {
    ...
    useEffect(() => {
        const fetchPlanData = async () => {
            try {
                const websiteId = "362ac74b-59fa-4e59-8269-5cf2edd85c99";
                const data = await getActivePlan(websiteId);
                setPlanData(data);
            } catch (error) {
    ...
    return (
        <div className=" justify-around bg-gradient-to-l from-[#1E212D] via-[#2E3A55] to-[#626C93] text-white flex items-center  p-4 relative z-50">
            {/* بخش چپ: دکمه پلن فعال */}
            {/* بخش چپ: دکمه پلن فعال */}
            <div className="flex-1 flex items-center ml-10 mr-6">
                <div className="relative">
                    <button className="bg-gradient-to-r from-[#EABF9F] to-[#D4A574] text-[#1E212D] px-4 py-2 rounded-lg font-semibold text-sm hover:from-[#D4A574] hover:to-[#EABF9F] transition-all duration-300 shadow-md flex items-center gap-2">
                        {!planData ? (
                            <>
                            ...
                            <>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>{getDaysRemaining(planData.expires_at) > 0 ? `${getDaysRemaining(planData.expires_at)} روز مونده` : 'منقضی شده'}</span>
                                </div>
                ...
                <div className="relative">
                    ...
                    {isProfileOpen && (
                        <div className="absolute left-0 mt-2 w-64 bg-white text-black rounded-lg shadow-lg p-4 z-[60]">
                            ...
                            <div className="text-sm text-gray-700 mb-4">seller@example.com</div>
                            <button
