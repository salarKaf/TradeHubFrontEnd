...
const ResetPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!newPassword.trim()) {
            ...
            newErrors.newPassword = 'رمز عبور باید شامل تمام الزامات باشد';
        }
        ...
        if (Object.keys(newErrors).length === 0) {
            setLoading(true);
            try {
                // شبیه‌سازی API call برای تغییر رمز عبور
                ...
                setErrors({
                    general: error.message || 'خطا در تغییر رمز عبور'
                });
            } finally {
                setLoading(false);
            }
                ...
                <div className="relative backdrop-blur-xl bg-white/10 border border-white/20 p-8 rounded-2xl shadow-2xl w-full max-w-md font-rubik">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* الزامات رمز عبور */}
                            {newPassword && (
                                <div className="bg-black/10 backdrop-blur-sm border border-black/20 rounded-xl p-4">
                                    <p className="text-black text-sm font-bold mb-3">الزامات رمز عبور:</p>
                                    <div className="space-y-2">
                                        {[
                                            { key: 'length', text: 'حداقل ۸ کاراکتر', valid: passwordRequirements.length },
                                            { key: 'uppercase', text: 'حداقل یک حرف بزرگ انگلیسی', valid: passwordRequirements.uppercase },
                                            { key: 'lowercase', text: 'حداقل یک حرف کوچک انگلیسی', valid: passwordRequirements.lowercase },
                                            { key: 'number', text: 'حداقل یک عدد', valid: passwordRequirements.number },
                                            { key: 'special', text: 'حداقل یک کاراکتر خاص', valid: passwordRequirements.special }
                                        ].map((req) => (
                                            <div key={req.key} className="flex items-center gap-2">
                                                ...
                                                <span className={`text-sm ${req.valid ? 'text-green-700' : 'text-black/70'}`}>
                                                    {req.text}
                                                </span>
                                            </div>
                                        ))}
